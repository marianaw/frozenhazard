"""Iterative stochastic imputation for Log-Normal AFT via TabPFN.
...
from src.fsa import (TABPFN_BATCH, TABPFN_TRAIN_MAX,
                     fit_sigma, survival_lognormal, predicted_median)
...
def _pseudo_obs_init(T, E, C_c, sigma=0.5):
    ...
    theta = compute_pseudo_obs(T, E, t0=t0)            # (n_train,) — all subjects
    unc   = E == 1
    # select only censored subjects
    theta_c = theta[~unc]
    theta_c = np.clip(theta_c, EPS, 1 - EPS)
    mu_c    = np.log(t0) - sigma * norm.ppf(1 - theta_c)
    return np.maximum(np.log(np.clip(C_c, EPS, None)), mu_c)


def _predict(X_ctx, log_y_ctx, X_query):
    """TabPFN in-context regression on log-times."""
    if len(X_ctx) > TABPFN_TRAIN_MAX:
        idx       = np.random.choice(len(X_ctx), TABPFN_TRAIN_MAX, replace=False)
        X_ctx     = X_ctx[idx]
        log_y_ctx = log_y_ctx[idx]
    with warnings.catch_warnings():
        warnings.filterwarnings("ignore", category=UserWarning, module="tabpfn")
        m = TabPFNRegressor(device="cpu")
        m.fit(X_ctx, log_y_ctx)
        return np.concatenate([
            m.predict(X_query[i : i + TABPFN_BATCH])
            for i in range(0, len(X_query), TABPFN_BATCH)
        ])


def run_stochastic_aft(X_u, y_u, X_c, C_c, X_test, T_test, E_test, t_grid,
                       n_iter=N_ITER, seed=0):
    ...
    _, _, sigma = run_pseudo_fsa(X_train, T_train, E_train, X_test, t_grid)
    ...
    for it in range(1, n_iter + 1):
        ...
        mu_all  = _predict(X_ctx, log_y_ctx, np.vstack([X_train, X_test]))
